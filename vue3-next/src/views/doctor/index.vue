<template>
	<div style="padding: 20px">
		...
		<el-table :data="tableData" @selection-change="selectionHandle">
			...
			<el-table-column label="预约" align="center" width="200px">
				<template #default="scope">
					<el-button type="success" round @click="handleOrder(scope.$index, scope.row)">预约</el-button>
				</template>
			</el-table-column>
		...
		<el-dialog v-model="dialogFormVisible2" title="预约">
			<el-form :model="form2" :inline="true">
				<el-form-item label="医生姓名" :label-width="formLabelWidth">
					<el-input v-model="form2.doc_name" autocomplete="off" />
				</el-form-item>
				<el-form-item label="医生电话" :label-width="formLabelWidth">
					<el-input v-model="form2.doc_phone" autocomplete="off" />
				</el-form-item>
				<el-form-item label="学生姓名" :label-width="formLabelWidth">
					<el-input v-model="form2.stu_name" autocomplete="off" placeholder="请输入你的姓名" />
				</el-form-item>

				<el-form-item label="学生电话" :label-width="formLabelWidth">
					<el-input v-model="form2.stu_phone" autocomplete="off" placeholder="请输入你的电话" />
				</el-form-item>
				<el-form-item label="预约时间" :label-width="formLabelWidth">
					<!-- <el-input v-model="form.pingwei" autocomplete="off" /> -->
					<div class="block">
						<span class="demonstration">:</span>
						<el-date-picker v-model="form2.order_time" type="datetime" placeholder="请选择你的预约时间" :default-time="defaultTime" />
					</div>
				</el-form-item>
			</el-form>
			<template #footer>
				<span class="dialog-footer">
					...
					<el-button type="primary" @click="addConfirm2"> 确定 </el-button>
				</span>
			</template>
		</el-dialog>
	</div>
...
function addConfirm() {
	if (titleindex.value == 0) {
		doctorInterface.addDoctor(form.value).then((res) => {
			if (res.code == 200) {
				ElMessage({
					message: '添加成功！',
					type: 'success',
		...
		doctorInterface.updateDoctor(form.value).then((res) => {
			if (res.code == 200) {
				ElMessage({
					message: '修改成功！',
					type: 'success',
...
function handleDelete(index, row) {
	...
	doctorInterface.deleteDoctor(delete1).then((res) => {
		if (res.code == 200) {
			ElMessage({
				message: '删除成功！',
				type: 'success',
...
/** 查询用户列表 */
function getList() {
	doctorInterface.doctor().then((res) => {
