<template>
	...
	<el-dialog v-model="dialogVisible" :title="menuData.menu_id ? '更新菜单' : '添加菜单'" width="500">
		<!-- 表单组件：添加或者更新已有的菜单数据 -->
		<el-form>
...
// 引入获取菜单请求APi
import { reqAllpermisstion, reqAdOrUpdateMenu, reqRemoveMenu, reqFindMenu } from '/@/api/menu';
// 引入ts类型
...
const save = async () => {
	...
	let result: any = await reqAdOrUpdateMenu(menuData);
	console.log(result);
	if (result.code == 200) {
		// 对话框隐藏
		dialogVisible.value = false;
		// 提示信息
		ElMessage({ type: 'success', message: menuData.menu_id ? '更新成功' : '添加成功' });
		// 再次获取全部最新的菜单的数据
		getHasPermisstion();
	} else {
		dialogVisible.value = false;
		ElMessage({ type: 'error', message: menuData.menu_id ? '更新失败' : '添加失败' });
	}
};
